...
// Gauge ring: ONE gauge series with N data items (ECharts "gauge-ring"
// pattern). Each data item draws a nested progress ring (progress.overlap
// splits the axisLine band into N rings) and carries its own title/detail
// positioned around the gauge via offsetCenter. items = [{code,name,pct}].
export function buildGaugeRingOption({ items }) {
  const N = items.length || 1
  const nameOf = Object.fromEntries(items.map((it) => [it.code, it.name]))
  const Rr = 56 // label ring radius (% of gauge radius)
  const data = items.map((it, i) => {
    const theta = (i * 2 * Math.PI) / N - Math.PI / 2 // top first, clockwise
    const lx = Rr * Math.cos(theta)
    const ly = Rr * Math.sin(theta) // +y = down
    ...
    return {
      ...
      name: it.code,
      title: {
        offsetCenter: [`${lx.toFixed(1)}%`, `${ly.toFixed(1)}%`],
        fontSize: 10,
        color: '#94a3b8',
      },
      detail: {
        offsetCenter: [`${lx.toFixed(1)}%`, `${(ly + 18).toFixed(1)}%`],
      },
      itemStyle: { color },
    }
  })
    ...
    series: [
      {
        ...
        progress: {
          ...
          roundCap: true,
          clip: false,
          itemStyle: { borderWidth: 1, borderColor: '#0b1220' },
        },
        axisLine: { lineStyle: { width: 34, color: [[1, '#1e2a45']] } },
