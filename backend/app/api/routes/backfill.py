...
- ``POST /backfill/cdp``  : Backfill CDP uptime from the oneminute WIDN
  history (file availability per minute).
- ``POST /backfill/dcp``  : Backfill DCP (sensor) data from oneminute files.
Both stream progress lines back so the Settings panel can show a live log.
"""
...
async def _iter_backfill_cdp(session: AsyncSession) -> AsyncSession:
    """Yield progress; CDP uptime derived from oneminute file availability."""
    ...
    while cursor < now:
        day = cursor.date()
        # One oneminute file per day; availability = file exists & parses.
        one_min = None
        for site in sites:
        ...
        for node in nodes:
            # upsert one connectivity row per day (reachable = file present)
...
async def _stream_cdp():
    async with AsyncSessionLocal() as session:
        async for line in _iter_backfill_cdp(session):
            yield line
        ...
        for site in sites:
            ...
            while cursor < now:
                ...
                if one_min and one_min.exists():
                    ...
                    for rec in recs:
                        for m in rec.metrics:
                            await session.execute(
                                ...
                                .values(
                                    time=rec.ts, sensor_id=m.sensor_id,
                    ...
                    # File missing => mark all site sensor metrics invalid for the minute.
                    for s in site_sensors:
                        if not s.is_enabled:
                            continue
                        await session.execute(
                ...
                cursor += timedelta(days=1)
                if cursor.minute % 15 == 0:  # flush periodically
                    await session.commit()
            await session.commit()
